...
function Nav() {
    ...
    // Close search when clicking/tapping outside the search box and icon.
    useEffect(() => {
        const handlePointerDown = (e) => {
            if (!showSearch) return

            const insideSearchBox = searchBoxRef.current?.contains(e.target)
            const insideIcon = searchIconRef.current?.contains(e.target)

            if (!insideSearchBox && !insideIcon) {
                setShowSearch(false)
            }
        }
    ...
    return (
        <div className='w-[100vw] h-[70px] bg-[#ecfafaec] z-10 fixed top-0 flex items-center justify-between px-[30px] shadow-md shadow-black'>
            ...
            <div className='w-[30%] flex items-center justify-end gap-[20px]'>
                {/* wrap icon(s) in span so ref.contains works reliably */}
                <span ref={searchIconRef} className='inline-block'>
                    {!showSearch &&
            ...
            {showSearch && (
                <div ref={searchBoxRef} className='w-[100%] h-[80px] bg-[#d8f6f9dd] absolute top-[100%] left-0 right-0 flex items-center justify-center'>
                    <input
                        type="text"
